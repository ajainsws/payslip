...
class PayrollController extends Controller
{
    ...
    public function postEditEmployeeSalary(Request $request)
    {
        ...
        if ($employee) {
            ...
            if($payment_type=='Hourly'){
                ...
                //print_r($request);
                //$employee = (object) array_merge((array) $employee, (array) $request);
                //print_r($employee);die;
                $employee->basic_salary=$request->basic_salary;
                $employee->hra=$request->hra;
                $employee->convey_allowance=$request->convey_allowance;
                $employee->medical_allowance=$request->medical_allowance;
                $employee->special_allowance=$request->special_allowance;
                $employee->education_allowance=$request->education_allowance;
                $employee->mobile_allowance=$request->mobile_allowance;
                $employee->miscellaneous=$request->miscellaneous;
                $employee->retirement_allowance=$request->retirement_allowance;
    ...
    public function postCustomSearch(Request $request)
    {
        ...
        $paid_employee_ids  = $employee_ids;
        ...
        return view('admin.make-payment', compact('payroll', 'employee', 'department', 'date', 'emp_id', 'dep_id', 'des_id', 'search_status','paid_employee_ids'));
    ...
    public function payPayment($emp_id,$date)
    {
        ...
        $text_date=date('F, Y',strtotime($date));
        $total_working_day = date('t',strtotime($date));
        ...
        if($payment_type=='Hourly'){
            ...
            $overtime_salary=round($overtime_salary,2);
            $net_salary=round(($employee->total_salary+$employee->basic_salary_increment),2);
        }
        $payment_amount=$net_salary+$overtime_salary;

        if((int)$net_salary <= (int)app_config('EsicMaxSalary')){
            $esic_amount = ceil(($net_salary * app_config('EsicEmployeePercent')) / 100);
        }else{
            $esic_amount = 0;
        }
        ...
        if($provident_fund){
            if($provident_fund->provident_fund_type=='Fixed Amount'){
                ...
                $provident_deducted=($employee->basic_salary*$provident_fund->employee_share)/100;
            }
        }else{
        ...
        $payment_amount=$payment_amount-$provident_deducted-$loan_deducted-$deducted_tax-$esic_amount;
        
        $payment_amount=round($payment_amount,2);



        return view('admin.pay-payment',compact('text_date','doj','total_working_day','net_salary','overtime_salary','payment_amount','emp_id','date','payroll','provident_deducted','loan_deducted','deducted_tax','esic_amount'));
    }

    /* payPaymentAll  Function Start Here */
    public function payPaymentAll($date)
    {
        $self='make-payment';
        if (\Auth::user()->user_name!=='admin'){
            $get_perm=permission::permitted($self);
        ...
        $employees=Employee::whereNotIn('id',$employee_ids)->where('user_name', '!=', 'admin')->get();

        /*$employees = DB::table('sys_employee')
            ->leftJoin('sys_payroll', 'sys_employee.id', '=', 'sys_payroll.emp_id')
            ->where('sys_payroll.payment_month',$text_date)
            ->whereNull('sys_payroll.emp_id')
            ->select('sys_employee.*','sys_payroll.emp_id')
            ->get();*/
        foreach($employees as $employee){
            ...
            $payroll=Payroll::firstOrCreate([
                ...
                'payment_month'=>$text_date,
                'payment_date'=>date('Y-m-d'),
                'total_working_days'=>$total_working_day,
                'present_working_days'=>$total_working_day,
                'net_salary'=>$net_salary,
                'tax'=> (isset($tax)) ? $tax : "0" ,
                'provident_fund'=>$provident_deducted,
                'loan'=>$loan_deducted,
                'esic' => $esic_amount,
                'overtime_salary'=>(isset($overtime_salary)) ? $overtime_salary : "0",
                'total_salary'=>$payment_amount,
                'deduction_amount'=>(isset($deduction_amount)) ? $deduction_amount : "0",
                'payment_type'=>'Bank Payment'
            ]);
    
            $month_date=date('Y-m',strtotime($payroll->payment_month));
            $first_day = date('Y-m-01', strtotime($month_date));
            $last_day = date('Y-m-t', strtotime($month_date));
    ...
    public function payPaymentPost(Request $request)
    {
        ...
        $payroll=Payroll::firstOrCreate([
            ...
            'payment_date'=>date('Y-m-d'),
            'total_working_days'=>$total_working_day,
            'present_working_days'=>$request->present_days,
            'net_salary'=>$request->net_salary,
            'tax'=> (isset($request->tax)) ? $request->tax : "0" ,
            'provident_fund'=>$provident_fund,
            'loan'=>$loan,
            'esic' => $request->esic_deducted,
            'overtime_salary'=>(isset($request->overtime_salary)) ? $request->overtime_salary : "0",
        ...
        if(count($loan_query)>0){
            foreach($loan_query as $l){
                ...
                if($l->remaining_amount>=$loan){
                    $l->remaining_amount-= $loan;
                }else{
                    $l->remaining_amount-=$l->remaining_amount;
                }
                $l->save();
    ...
    public function postPayslipCustomSearch(Request $request)
    {
        ...
        $payroll=Payroll::where('payment_month',$text_date);
    ...
    public function deleteLoan($id)
    {
     ...
     /* deletePayroll  Function Start Here */
     public function deletePayroll($id)
     {
         ...
         $loan_query=Loan::where('emp_id',$payroll->employee_info->id)->where('enable_payslip','yes')->get();
        if(count($loan_query)>0){
            foreach($loan_query as $l){
                $l->remaining_amount+= $l->repayment_amount;
                $l->save();
    ...
    public function downloadPdf($id)
    {
      ...
      // dd($html);
        ...
        //$pdf->loadHTML($html);
        //return $pdf->inline();
        //PDF::setOptions(['dpi' => 600, 'defaultFont' => 'sans-serif','debugCss'=> true]);
        $pdf = \App::make('dompdf.wrapper');
        $pdf->loadHTML($html);
        return $pdf->stream();
        //$pdf = PDF::loadView('admin.pdf-payslip', $html);
