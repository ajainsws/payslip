                    ...
                    <div class="panel">
                        ...
                        <div class="panel-body">
                            <form class="" role="form" method="post" action="{{url('payroll/make-payment/post-custom-search')}}">
                                ...
                                <button type="button" class="btn btn-primary pull-right" id="makePaymentAll"><i class="fa fa-credit-card"></i> Generate All</button>
                                <button type="submit" class="btn btn-success pull-right" style="margin-right: 10px;"><i class="fa fa-search"></i> {{language_data('Search')}} </button>
                                ...
                                @foreach($payroll as $d)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @elseif(in_array($d->id,$paid_employee_ids))
                                                <p>Already Paid</p>
                                            @else
                                            <a href="#" id="{{$d->id}}" class="btn btn-success btn-xs makePayment"><i class="fa fa-credit-card"></i> {{language_data('Make Payment')}}</a>
...
@section('script')
    ...
    <script>
        $(document).ready(function () {
            ...
            $("#makePaymentAll").click(function (e) {
                e.preventDefault();
                if($(".monthPicker").val() == ""){
                    $(".monthPicker").focus();
                }else{
                    var month = $(".monthPicker").val();
                    bootbox.confirm("Are you sure to generate Payslip for "+month+" ?", function (result) {
                        if (result) {
                            var _url = $("#_url").val();
                            window.location.href = _url + "/payroll/pay-payment-all/" + month ;
                        }
                    });
                }
            });
